 ...
 * Using the Medplum FHIR types for better standardization.
 */
...
import { validateFhirResource } from './client'
...
export function callVariablesToFHIR(callVariables: typeof defaultCallVariables): Bundle {
  ...
  const patient: Patient = {
    ...
    id: patientId,
    identifier: [
      createIdentifier('urn:vitalus:patients', patientId)
    ],
    active: true,
    name: [
      createHumanName(callVariables.patient.name)
    ],
    address: callVariables.patient.address ? [
      {
        use: 'home',
  ...
  const practitioner: Practitioner = {
    ...
    id: practitionerId,
    identifier: [
      createIdentifier('urn:vitalus:practitioners', practitionerId)
    ],
    active: true,
    name: [
      createHumanName(callVariables.doctor.name)
    ]
  }
  ...
  try {
    ...
    const timeStr = callVariables.appointment.morningSlot || callVariables.appointment.afternoonSlot // e.g., "9am"
  ...
  const coverage: Coverage = {
    ...
    id: coverageId,
    status: 'active',
    type: createCodeableConcept('Health Insurance', [
      {
    ...
    beneficiary: createReference('Patient', patientId, callVariables.patient.name),
    payor: [
      {
        display: callVariables.payment.insuranceName
      }
    ],
    costToBeneficiary: callVariables.payment.insuranceCoverage ? [
      {
        type: {
  ...
  const condition: Condition = {
    ...
    id: conditionId,
    subject: createReference('Patient', patientId, callVariables.patient.name),
    code: createCodeableConcept(callVariables.patient.condition),
    note: callVariables.patient.lastVisit ? [
      {
    ...
    for (const entry of bundle.entry) {
      ...
      // Handle Condition resource
        ...
        if (condition.note && condition.note.length > 0) {
          ...
          if (note && note.includes('Last visited')) {
...
export function extractFHIRFromText(extractedText: any): Bundle {
  try {
    // If we already have a FHIR Bundle, validate it and return
    if (extractedText && extractedText.resourceType === 'Bundle') {
      const validationResult = validateFhirResource(extractedText)
      if (validationResult.valid) {
        return extractedText as Bundle
      }
      console.warn('Invalid FHIR Bundle:', validationResult.errors)
    }
    ...
    console.error('Error extracting FHIR from text:', error)
